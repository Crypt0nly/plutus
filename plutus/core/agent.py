    ...
    def _build_system_prompt(self) -> str:
        ...
        if cloud_cfg.token and cloud_cfg.enabled:
            ...
            parts.append(
                ...
                "### Receiving Messages from Cloud (cloud-initiated)\n"
                ...
                "agent as a TOOL RESULT (the cloud agent sees your reply as the output "
                "of its `send_message` tool call, not as a new incoming message).\n\n"
                "### Sending Messages to Cloud (local-initiated)\n"
                "You can proactively send messages to the cloud agent using:\n"
                + curl_example
                + "\n"
                "This is a SYNCHRONOUS call: it sends your message and WAITS for the "
                "cloud agent's reply. The reply is returned in the HTTP response body "
                'as `{"status": "ok", "reply": "..."}`. You see the reply as '
                "a tool result (the output of your curl command), NOT as a new incoming "
                "message. This means NO LOOP is possible.\n\n"
                ...
                "### CRITICAL \u2014 No Loops Possible\n"
                "The messaging system is designed so loops CANNOT happen:\n"
                "- When cloud sends you a message, your reply goes back as a TOOL "
                "RESULT to the cloud agent. Tool results do not trigger new agent "
                "turns. One message in, one reply out, done.\n"
                "- When you send a message to cloud via the curl endpoint, the cloud "
                "agent's reply comes back in the HTTP response. You see it as a tool "
                "output. No new message is created. One send, one reply, done.\n"
