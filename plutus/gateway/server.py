...
async def _worker_executor(task: WorkerTask, on_status: Any, *, deadline: float | None = None) -> str:
    ...
    worker_system_prompt = (
        "You are a Plutus worker agent running on a Windows PC. You have been assigned a "
        "specific task by the coordinator. Complete it efficiently using the available tools.\n\n"
        "BEHAVIOR RULES:\n"
        ...
        "- To CREATE or WRITE files: Use the 'filesystem' tool with operation='write', "
        "path='C:\\\\path\\\\to\\\\file.ext', content='full file content here'. "
        "This is the MOST RELIABLE way to create files. Do NOT use code_editor for simple writes.\n"
        "- To READ files: Use 'filesystem' with operation='read', path='...'.\n"
        "- To RUN commands: Use 'shell' with command='your command'. This runs in PowerShell on Windows.\n"
        "  For multi-line scripts, write a .ps1 file first with 'filesystem', then run it with 'shell'.\n"
        "  Avoid passing large HTML/code as inline shell arguments — write to a file instead.\n"
        "- To OPEN files in a browser: Use 'shell' with command='start msedge \"C:\\\\path\\\\to\\\\file.html\"'.\n"
        "- To OPEN files in default app: Use 'shell' with command='start \"\" \"C:\\\\path\\\\to\\\\file.ext\"'.\n"
        "- The 'code_editor' tool uses a Python subprocess — prefer 'filesystem' for simple file ops.\n"
        ...
        "WORKFLOW for creating files (HTML, scripts, etc.):\n"
        "  Step 1: Use filesystem(operation='write', path='...', content='full content')\n"
        "  Step 2: Optionally use shell to open or run the file\n"
