...
import { Brain, Save, Key, Eye, EyeOff, CheckCircle2, AlertTriangle, Trash2, ChevronDown } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#1a1a2e] rounded-xl border border-gray-800/60 p-5">
            ...
            {providers.map((p) => (
              <button
                ...
                className={`relative flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${
                  provider === p.id
                    ? "border-plutus-500/50 bg-plutus-500/5 shadow-sm shadow-plutus-500/10"
                    : "border-gray-800/60 bg-gray-800/20 hover:border-gray-700/60 hover:bg-gray-800/40"
                }`}
              >
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${p.color} flex items-center justify-center text-white text-xs font-bold shadow-sm`}>
                  {p.icon}
                </div>
                <span className="text-xs font-medium text-gray-300">{p.label}</span>
                <span className="text-[10px] text-gray-500">{p.sublabel}</span>
                {provider === p.id && (
                  <div className="absolute -top-px -right-px w-3 h-3 bg-plutus-500 rounded-full rounded-tl-none border-2 border-[#1a1a2e]" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="mb-5">
          ...
          {models.length > 0 ? (
            <div className="space-y-1.5">
              {models.map((m) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${
                    model === m.id
                      ? "border-plutus-500/40 bg-plutus-500/5"
                      : "border-gray-800/40 bg-gray-800/20 hover:border-gray-700/40"
                  }`}
                >
                  <div className={`w-2 h-2 rounded-full shrink-0 ${
                    model === m.id ? "bg-plutus-400" : "bg-gray-600"
                  }`} />
              ...
              >
                <div className={`w-2 h-2 rounded-full shrink-0 ${model === "" ? "bg-plutus-400" : "bg-gray-600"}`} />
                <span className="text-sm text-gray-400">Custom model...</span>
              </button>
              {model === "" && (
                <input
                  type="text"
                  className="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-plutus-500/50 focus:ring-1 focus:ring-plutus-500/20 ml-5"
                  value={customModel}
        ...
        <div className="grid grid-cols-2 gap-4 mb-5">
          ...
          <div>
            <label className="text-xs text-gray-500 mb-1.5 block">Max Tokens</label>
            <input
              type="number"
              className="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-plutus-500/50 focus:ring-1 focus:ring-plutus-500/20"
              value={maxTokens}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            {hasKey && (
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/15 px-2.5 py-1.5 rounded-lg transition-colors"
              >
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                className="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg px-3 py-2.5 pr-10 text-sm text-gray-300 focus:outline-none focus:border-plutus-500/50 focus:ring-1 focus:ring-plutus-500/20"
                value={apiKey}
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-500 disabled:opacity-50 disabled:hover:bg-amber-600 text-white text-sm font-medium transition-colors whitespace-nowrap"
            >
