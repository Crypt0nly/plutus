...
export async function downloadWorkspaceFile(path: string, suggestedName?: string): Promise<void> {
  ...
  const encodedPath = path
    ...
    .join("/");
  const res = await fetch(`${BASE}/workspace/download/${encodedPath}`, {
    headers,
  });
  if (!res.ok) {
    const text = await res.text();
  ...
  a.href = url;
  a.download = suggestedName || path.split("/").filter(Boolean).pop() || "download";
...
export const api = {
  ...
  getMessages: (convId: string) =>
    safeRequest<Record<string, unknown>>(`/chat/${convId}`, { conversation: null, messages: [] }),
  ...
  // Scheduler — DB-backed cloud scheduler at /scheduler/*
  getScheduler: () => safeRequest<Record<string, any>>("/scheduler/jobs", { jobs: [] }),
  getScheduledJobs: () => safeRequest<Record<string, any>>("/scheduler/jobs", { jobs: [] }),
  getScheduledJob: (jobId: string) =>
    safeRequest<Record<string, any>>(`/scheduler/jobs/${jobId}`, {}),
  ...
  getUsage: (month?: string) =>
    safeRequest<Record<string, any>>(`/billing/usage${month ? `?month=${month}` : ""}`, {
      billing_month: "", total_input_tokens: 0, total_output_tokens: 0,
      total_normalized_tokens: 0, by_model: {}, plan: "free", limit: 500000, limit_reached: false,
    }),
  ...
  createVoiceSession: (voice?: string) =>
    request<{ client_secret: string; model: string; voice: string; available_voices: string[] }>(
      "/voice/session",
      {
        method: "POST",
  ...
  executeVoiceTool: (toolName: string, args: Record<string, unknown> = {}) =>
    request<{
      ok: boolean;
      tool_name: string;
      output: string;
  ...
  triggerZernioBackfill: () =>
    ...
    }>("/zernio/admin/backfill", { method: "POST" }),
  ...
  /** Check if the user's local PC bridge is connected to the cloud. */
  getLocalBridgeStatus: () =>
    safeRequest<{ connected: boolean }>("/bridge/status", { connected: false }),

  /**
   * Fetch a one-liner install script for the given platform.
   * The caller's Clerk session token is injected so the script is
   * fully self-contained — the user just pastes it and runs it.
   */
  getBridgeInstallScript: async (platform: "macos" | "linux" | "windows", token: string): Promise<string> => {
    const BASE = (import.meta as any).env?.VITE_API_BASE_URL ?? "http://localhost:8000/api";
    const url = `${BASE}/bridge/install-script?platform=${platform}`;
    const res = await fetch(url, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
    // Replace the __TOKEN__ placeholder with the real Clerk JWT
    const raw = await res.text();
    return raw.replace(/__TOKEN__/g, token);
  },
