...
expected = {
    "users": sorted(["id", "email", "display_name", "avatar_url", "plan", "is_active",
              "settings", "connector_credentials", "last_seen_at", "created_at", "updated_at"]),
    "agent_states": sorted(["id", "user_id", "status", "current_task", "execution_context",
                     "bridge_connected", "last_heartbeat", "created_at", "updated_at"]),
    "memories": sorted(["id", "user_id", "category", "content", "metadata", "sync_version",
                 "synced_at", "created_at", "updated_at"]),
    "skills": sorted(["id", "user_id", "name", "description", "skill_type", "definition",
               "is_shared", "sync_version", "created_at", "updated_at"]),
    "scheduled_tasks": sorted(["id", "user_id", "name", "description", "schedule", "prompt",
                        "is_active", "last_run", "next_run", "created_at", "updated_at"]),
    "conversations": sorted(["id", "user_id", "title", "is_active", "metadata",
                       "created_at", "updated_at"]),
    "messages": sorted(["id", "conversation_id", "user_id", "role", "content",
                 "tool_calls", "token_count", "created_at", "updated_at"]),
    "sync_log": sorted(["id", "user_id", "entity_type", "entity_id", "action",
                 "data", "sync_version", "source", "created_at", "updated_at"]),
}
...
expected_fks = {
    ("user_id", "users.id"): 7,  # agent_states, memories, skills, scheduled_tasks, conversations, messages, sync_log
